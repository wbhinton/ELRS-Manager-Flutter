...
void main() {
  group('DeviceEditorViewModel Tests', () {
    ...
    setUp(() {
      ...
      baseConfig = const RuntimeConfig(
        ...
        config: ElrsConfig(
          vbind: 0,
          serialProtocol: 0,
          hardware: {'type': 'rx', 'has_pwm': false},
        ),
      );
    });
    ...
    test('Updating an option (domain) flags hasChanges as true', () {
      viewModel.initialize(baseConfig);
      viewModel.updateOption('domain', 1);
      ...
      expect(container.read(deviceEditorProvider)?.options.domain, 1);
    });
    ...
    test(
      'saveChanges correctly orchestrates saving logic depending on whats changed',
      () async {
        viewModel.initialize(baseConfig);
        ...
        Future<void> mockSaveOptions(
          String ip,
          Map<String, dynamic> opts,
        ) async => savedOptions = true;
        Future<void> mockSaveConfig(
          String ip,
          Map<String, dynamic> cfg,
        ) async => savedConfig = true;
    ...
    test('saveChanges triggers both endpoints if both are changed', () async {
      ...
      Future<void> mockSaveConfig(String ip, Map<String, dynamic> cfg) async =>
          savedConfig = true;
      Future<void> mockReboot(String ip) async {}

      viewModel.updateOption('wifi-ssid', 'NewNet');
      viewModel.updateConfigValue('serial-protocol', 1);
