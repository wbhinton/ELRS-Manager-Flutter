...
class TargetResolver {
  ...
  static Map<String, dynamic> resolveLayout(
    ...
    final firmwareStr = (targetConfig['firmware'] as String? ?? '').toUpperCase();
    String? folder;
    ...
    if (file == null) {
      // Try exact path provided in config (if it had a folder)
    ...
    if (file == null) {
      ...
      try {
        file = hardwareZip.files.firstWhere((f) => f.name.endsWith('/$basename') || f.name == basename);
      } catch (e) {
        // Not found
        file = null;
      }
    ...
    if (file == null) {
       throw Exception('Could not find layout "$layoutFile" in hardware archive (checked RX/, TX/, and full scan).');
    }
